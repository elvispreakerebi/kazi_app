...
export const getTeacherLessonPlansHandler = async (ctx: any, req: Request) => {
  ...
  try {
    const lessonPlans = await ctx.runQuery(api.functions.lessonPlans.getTeacherLessonPlans.getTeacherLessonPlans, {
      teacherId: verify.teacherId as Id<"teachers">,
    });
    return new Response(JSON.stringify(lessonPlans), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (err) {
